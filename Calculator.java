...
public class Calculator {
	...
	public static boolean isValid(String s) {
		...
		boolean ret = true;
		/*int prev = 0;	// 0 for num, 1 for operand
		int curr = 1;
		for(int i=0; i<s.length(); i++) {
			if(!s.substring(i, i+1).matches("[0-9"+operands+"]"))
				return false;
			
			if(i>0) {
				if(s.substring(i, i+1).matches("["+operands+"]"))
					curr = 1;
				else
					curr = 0;
	...
	public static String fixPosNegMult(String s) {
		...
		for(int i=1; i<s.length()-1; i++) {
			if(s.substring(i, i+1).matches("["+multDiv+"]") && s.substring(i+1, i+2).matches("["+addSub+"]")) {
				double n1=0;
				double n2=0;
	...
	public static String fixPowers1(String s) {
		...
		for(int i=1; i<s.length(); i++) {
			if(s.charAt(i) == '^' && (s.charAt(i+1) == '-' || s.charAt(i+1) == '+') && i<s.length()) {
				...
				int min = i-1;
				for(int j = min; j>=0; j--) {
					/*String test1 = "[0-9"+decimal+parenOpen+"]";
					String test2 = "[0-9"+decimal+parenClose+"]";
	...
	public static boolean validSpaces(String s) {
		// prevents numbers from concatenating
			s = s.trim();
			for(int i=1; i<s.length(); i++) {
				if(s.charAt(i)==' ' && s.substring(i-1,i).matches("[0-9]")) {
	...
	public static String getResult(String s) {
		try {
			...
			if(!isValid(s))
				throw new Exception();
			else return Double.toString(calcGroups(s));
		}
		catch(Exception ex) {
			//ex.printStackTrace();
			return "ERROR";
...
/*public class CalculatorGUI2 extends Application {
	...
	public void start(Stage primaryStage) {
		File f = new File("calcStyle.css");
		
		Label inputLabel = new Label("Function: ");
		TextField inputText = new TextField();
		inputText.setPrefWidth(300);
		Button enter = new Button("Enter");
		Label previous = new Label("Previous: ");
		
		ScrollPane ansScroll = new ScrollPane();
		ansScroll.setContent(previous);
		ansScroll.setHmax(300);
		ansScroll.setVbarPolicy(ScrollBarPolicy.NEVER);
		
		enter.setOnAction(new EventHandler<ActionEvent>() {
			
			public void handle(ActionEvent e) {
				String function = inputText.getText();
		...
		textPane.getChildren().addAll(inputLabel, inputText);
		//textPane.setAlignment(Pos.CENTER);
		
		VBox inputPane = new VBox();
		inputPane.getChildren().addAll(textPane, enter, previous);
		inputPane.setAlignment(Pos.CENTER);
		VBox.setMargin(textPane, new Insets(50,20,10,20));
		VBox.setMargin(enter, new Insets(10,20,20,20));
		VBox.setMargin(previous, new Insets(10,20,20,20));
		
		Scene inputScene = new Scene(inputPane);
		//File f = new File("calcStyle.css");
		inputScene.getStylesheets().clear();
		inputScene.getStylesheets().add("file:///" + f.getAbsolutePath().replace("\\", "/"));						
	...
	public static void main(String[] args) {
		/*try {
			...
			System.out.println(Calculator.fixGroups("(1/2)"));
		}
		catch(Exception ex) {
